  ...
  return (
    <div className="space-y-4">
      ...
      {libraryComponents.length === 0 ? (
        ...
        <div className="grid grid-cols-2 gap-3 max-h-80 overflow-y-auto py-2 pr-1">
          {libraryComponents.map((item) => (
            <div
              key={item.id}
              onClick={() => handleLibraryItemClick(item)}
              className={`cursor-pointer transition-all duration-200 mx-auto p-1 rounded-md border ${
                selectedLibraryItem?.id === item.id
                  ? "ring-1 ring-neutral-700 bg-neutral-200"
                  : "border-neutral-200 hover:bg-neutral-200 "
              }`}
            >
              <SmallPreviewContainer container={item.container} />
              <div className="p-2 text-center">
                <p className="text-xs text-neutral-600 truncate">{item.name}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <button
        ...
        className={`w-full px-3 py-3 text-xs font-medium rounded transition-colors ${
          selectedLibraryItem && selectedContainerId
            ? "bg-neutral-800 text-white hover:bg-neutral-900 button-box"
